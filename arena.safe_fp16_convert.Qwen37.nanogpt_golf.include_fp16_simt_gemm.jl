...
# === 1. Проверка LLVM аргументовъ для запрета FMA contraction ===
...
using CUDA
using CUDA.CUBLAS
include("fp16_simt_gemm.jl")

# Запрещаемъ скалярный индексингъ на GPU для гарантіи производительности
CUDA.allowscalar(false)

# ============================================================
# Улучшенная функція to_scalar (защита отъ Array(Float32))
# ============================================================
function to_scalar(x)
    # Если уже число - просто конвертируемъ
    if x isa Number
        return Float32(x)
    end
    
    # Если массивъ - извлекаемъ первый элементъ
    if x isa AbstractArray
        try
            # Для GPU массивовъ - переносимъ на CPU
            if x isa CUDA.CuArray
                arr = Array(x)
                # Обработка 0-D массивовъ и скаляровъ
                if ndims(arr) == 0 || length(arr) == 0
                    return Float32(arr[])
                else
                    return Float32(arr[1])
                end
            elseif ndims(x) == 0 || length(x) == 0
                return Float32(x[])
            else
                return Float32(x[1])
            end
        catch e
            @warn "to_scalar: не удалось извлечь изъ массива: $e"
            return Float32(0)
        end
    end
    
    # Попытка прямого преобразованія (для TrackedReal и подобныхъ)
    try
        return Float32(x)
    catch
        # Послѣдняя попытка - черезъ Array
        try
            arr = Array(x)
            if ndims(arr) == 0 || length(arr) == 0
                return Float32(arr[])
            ...
            @warn "to_scalar: полный провалъ для типа $(typeof(x)): $e"
            return Float32(0)
        end
    end
end

# === Подключеніе компрессоровъ ===
include("TQ3_1S.jl")
using .TQ3_1S

# === Конфигурація гибриднаго сжатія ===
const HYBRID_CKPT_USE_TQ3 = Ref{Bool}(true)
const HYBRID_CKPT_TQ3_MIN_SIZE = Ref{Int}(100_000)

# NanoGPT-Golf v5.5 — Optimized for NVIDIA CMP 50HX (Turing SM 7.5)
#function force_cublas_no_tensorcores!()
#    try
 #       handle = CUBLAS.handle()
        # Попытка установить режимъ, исключающій тензорныя ядра
  #      if isdefined(CUBLAS, :CUBLAS_DEFAULT_MATH)
   #         CUBLAS.cublasSetMathMode(handle, CUBLAS.CUBLAS_DEFAULT_MATH)
    #    end
        # Явное запрещеніе Tensor Cores черезъ переменную окруженія
     #   ENV["CUDA_DISABLE_TENSOR_CORES"] = "1"  # ← Критическое исправленіе!
      #  @info "✅ cuBLAS: Tensor Cores explicitly disabled for CMP 50HX"
    #catch e
     #   @warn "⚠️ Не удалось установить режимъ cuBLAS: $e"
    #end
#end
#force_cublas_no_tensorcores!()

ENV["CUDA_DISABLE_TENSOR_CORES"] = "1"  # ← Измѣнено съ "0" на "1"
CUDA.versioninfo()
...
function verify_gpu_math_performance()
 @info "⚠️ CMP 50HX: cuBLAS FP16 is broken. Using custom SIMT kernel."
 ok = verify_custom_fp16_gemm()
 if !ok
 @warn "FP16 GEMM verification failed. Training will use custom kernel but correctness is not guaranteed."
 end
end
...
DEV(x) = HAS_CUDA ? gpu(x) : cpu(x)

# ============================================================
# Безопасная конвертація FP32 → FP16
# ============================================================
function safe_fp16_convert(x)
    if x isa AbstractArray{<:AbstractFloat}
        # Clamp къ безопасному діапазону для Float16: [-50000, 50000]
        # (максимумъ Float16 = 65504, но запасъ нуженъ для ариѳметики)
        clamped = clamp.(x, Float32(-50000), Float32(50000))
        # Проверка на конечность передъ конвертаціей
        if any(!isfinite, clamped)
            @warn "safe_fp16_convert: обнаружены неконечные значения, заменяю на 0"
            clamped = ifelse.(isfinite.(clamped), clamped, Float32(0))
        end
        return Float16.(clamped)
    elseif x isa AbstractFloat
        val = Float32(x)
        val = clamp(val, Float32(-50000), Float32(50000))
        return isfinite(val) ? Float16(val) : Float16(0)
    else
        return x  # Не числовые типы не трогаемъ
    end
end
...
# ============================================================
# INT8 Компрессоръ (SM56-Defoamer)
# ============================================================
# gpt-5.4-search: Сделать sm56_compress устойчивым к NaN/Inf
function sm56_compress(x::CuArray{Float16})
    n = length(x)
    n == 0 && return CuArray{Int8}(), Float16(0.0), size(x)

    x_f32 = Float32.(x)

    # Санитизация: NaN/Inf -> 0
    x_safe = ifelse.(isfinite.(x_f32), x_f32, 0f0)

    m_val = to_scalar(CUDA.maximum(abs, x_safe))
    m_val = (!isfinite(m_val) || m_val <= 0f0) ? 1f-8 : m_val

    scale_f32 = max(m_val / 127.0f0, 1f-8)
    qf = clamp.(round.(x_safe ./ scale_f32), -127f0, 127f0)
    q = Int8.(qf)

    CUDA.synchronize()  # чтобы ошибка падала здесь, а не на Array(q)

    return q, Float16(scale_f32), size(x)
end
...
#    m_val = to_scalar(CUDA.maximum(abs, x))
 #   m_val = (!isfinite(m_val) || m_val <= 0f0) ? 1f-8 : m_val

  #  scale = Float16(m_val / 127.0f0)
   # q = Int8.(clamp.(round.(Float32.(x) ./ Float32(scale)), -127f0, 127f0))

    #return q, scale, size(x)
#end
# Почему это лучше
  #  • не делает лишний Float32.(abs.(x))
   # • не аллоцирует временный массив
    #• не зависит от того, вернул CUDA.maximum скаляр или 0-d массив


#    x_abs_f32 = Float32.(abs.(x))
 #   m = CUDA.maximum(x_abs_f32)
  #  m_val = to_scalar(m)  # ✅ ИСПРАВЛЕНИЕ: замена Array(m)[1] на to_scalar(m)
   # m_val = (!isfinite(m_val) || m_val <= 0f0) ? 1f-8 : m_val
    #scale = Float16(m_val / 127.0f0)
...
function nll_loss(logits::AbstractMatrix, targets::AbstractVecOrMat{<:Integer})
    ...
    idx = flat_t .+ (0:N-1) .* V
    base_loss = -sum(lp[idx]) / Float32(N)
    z_loss_part = 0.0f0
    if Z_LOSS_COEF[] > 0
        vars = var(logits32; dims=1)
        z_loss_part = -sum(log.(vars .+ Float32(1e-8))) / Float32(length(vars))
    end
    return base_loss + Z_LOSS_COEF[] * z_loss_part
end

# ============================================================
# Stable NLL Loss for FP16 training (always computes in FP32)
# ============================================================
function stable_nll_loss_fp16(logits::AbstractMatrix, targets::AbstractVecOrMat{<:Integer})
    # Always promote to FP32 for numerical stability in logsoftmax
    logits_f32 = Float32.(logits)
    flat_t = vec(targets)
    N = length(flat_t)
    V = size(logits_f32, 1)
    
    # Clamp logits to safe range before logsoftmax
    logits_clamped = clamp.(logits_f32, -100f0, 100f0)
    
    # Compute logsoftmax in FP32
    lp = Flux.logsoftmax(logits_clamped; dims=1)
    idx = flat_t .+ (0:N-1) .* V
    base_loss = -sum(lp[idx]) / Float32(N)
    
    # Z-loss component (also in FP32)
    z_loss_part = 0f0
    if Z_LOSS_COEF[] > 0
        vars = var(logits_f32; dims=1)
        z_loss_part = -sum(log.(vars .+ Float32(1e-8))) / Float32(length(vars))
    end
    return base_loss + Z_LOSS_COEF[] * z_loss_part
end
...
# ============================================================
# Tree utils & Safe Scalar Extraction
# ============================================================
"""Безопасное извлеченіе скаляра безъ trigger'а scalar indexing"""
...
function tree_all_finite(x)::Bool
    ...
    Functors.fmap(x) do v
        if v isa AbstractArray
            try
                # Переносимъ на CPU для провѣрки, избѣгая GPU-скаляровъ
...
function tree_max_abs(x)::Float32
    ...
    Functors.fmap(x) do v
        if v isa AbstractArray && length(v) > 0
            try
                v_f32 = v isa CuArray{Float16} ? Float32.(v) : v
                m = to_scalar(CUDA.maximum(abs, v_f32))
                mx[] = max(mx[], m)
            catch _
                mx[] = Inf32
            end
...
function grad_l2norm(gs)::Float32
    ...
    Functors.fmap(gs) do g
        g isa AbstractArray && (s[] += to_scalar(CUDA.sum(abs2, g)))
        g
    end
    sqrt(s[])
end
...
function parse_cmd()
    ...
    @add_arg_table s begin
        ...
        "--skip-smoke-test"; action=:store_true  # ← Добавлено
    end
    ...
    if !args["autotune-benchmark"] && !args["no-autotune-benchmark"]; args["autotune-benchmark"] = true; end
    args["no-autotune-benchmark"] && (args["autotune-benchmark"] = false)
    args["lr-min"] = max(args["lr-min"], 1e-7)
    if !args["qat-per-row"] && !args["qat-per-tensor"]; args["qat-per-row"] = true; end
    args["qat-per-tensor"] && (args["qat-per-row"] = false)
    if !args["fp16"] && !args["no-fp16"]; args["fp16"] = HAS_CUDA; end
...
function push_gnorm!(v::Float32)
    ...
    while length(RECENT_GNORMS) > RECENT_GNORMS_MAX; popfirst!(RECENT_GNORMS); end
end
...
function refill!(ld::ByteLoader, need::Int)
    ...
    while loader_available(ld) < target
        if eof(ld.io); try seekstart(ld.io) catch end; break; end
        local line; read_ok = true
        try line = readline(ld.io) catch; read_ok = false; end
        !read_ok && break
        isempty(line) && continue
        txt = try string(JSON3.read(line)[:text]) catch _; line end
        isempty(txt) && continue
...
function reset_offload_stats!()
    OFFLOAD_BYTES_TO_CPU[] = 0; OFFLOAD_BYTES_TO_GPU[] = 0
    OFFLOAD_TIME_CPU_S[]   = 0.0; OFFLOAD_TIME_GPU_S[]   = 0.0
    OFFLOAD_COUNT[]        = 0
end
...
function is_weight_like(x::CuArray{Float16})
    ndims(x) == 3 && return false
    length(x) < HYBRID_CKPT_TQ3_MIN_SIZE[] && return false
    return ndims(x) <= 2
end

function compress_adaptive(x::CuArray{Float16})
    shape = size(x)
    if HYBRID_CKPT_USE_TQ3[] && is_weight_like(x)
        data, scales, _ = TQ3_1S.tq3_1s_compress(x)
        # scales is already a CuArray{Float16}, convert to CPU array
        payload = (Array(data), Array(scales))
        return (:TQ3_1S, payload, shape)
    else
        q, scale, _ = sm56_compress(x)
        # scale is a scalar Float16; store as 1-element vector for uniformity
        payload = (Array(q), [scale])
        return (:INT8, payload, shape)
    end
end
function decompress_adaptive(method::Symbol, payload, shape::Dims)
    if method === :TQ3_1S
        cpu_data, cpu_scales = payload
        gpu_data = CuArray(cpu_data)
        gpu_scales = CuArray(cpu_scales)
        return TQ3_1S.tq3_1s_decompress(gpu_data, gpu_scales, shape)
    else
        cpu_q, cpu_scale_vec = payload
        gpu_q = CuArray(cpu_q)
        scale = cpu_scale_vec[1]  # extract scalar from vector
        return sm56_decompress(gpu_q, scale, shape)
    end
end

function compressed_bytes(method::Symbol, payload)
    return sizeof(payload[1]) + sizeof(payload[2])
end


# gpt-5.4-search: Я бы ещё добавил защиту в hybrid_checkpoint
# Чтобы компрессор не валил всю тренировку:

@adjoint function hybrid_checkpoint(f, x)
    if !HYBRID_CKPT_ENABLED[] || !(x isa CuArray{Float16})
    ...
    try
        method, payload, orig_shape = compress_adaptive(x)
    catch e
        @warn "hybrid_checkpoint compression failed; disabling checkpoint fallback: $e"
    ...
    function back(Δ)
        x_reloaded = decompress_adaptive(method, payload, orig_shape)
...
function compute_scale_sym(W; bits::Int, per_row::Bool, eps::Float32=1f-8)
    qm = qmax_int(bits)
    if per_row
        mx = vec(CUDA.maximum(abs, Float32.(W); dims=2))
        return max.(mx ./ qm, eps)
    else
        mx = to_scalar(CUDA.maximum(abs, W))
        return max(mx / qm, eps)
    end
end
...
(r::RMSNorm)(x) = x ./ sqrt.(sum(abs2, x; dims=1) ./ size(x,1) .+ 1f-6) .* r.scale
...
function (a::GQAttention)(x)
    ...
    if a.mode == "flash"
        q4 = permutedims(q, (1,3,2,4)); k4 = permutedims(k, (1,3,2,4)); v4 = permutedims(v, (1,3,2,4))
...
function zeropower_ns5(G; steps::Int=5)
    ...
    return Float32.(reshape(X, sz)) .* Float32.(G_norm)
end
...
function Optimisers.apply!(o::NorMuon, st::NMState, x::AbstractArray, dx)
    ...
    if T == Float16; upd = clamp.(upd, Float16(-1000), Float16(1000)); end
    upd = T.(zeropower_ns5(upd; steps=o.ns_steps))
    nd2 = ndims(upd)
    vn    = sqrt.(CUDA.sum(upd.^2; dims=(nd2-1, nd2)) .+ ε)
    vm    = sum(upd.^2; dims=nd2) ./ size(upd, nd2)
    v_new = β2 .* st.v .+ (T(1) - β2) .* vm
    upd  = upd ./ sqrt.(v_new .+ ε)
    vn2  = sqrt.(CUDA.sum(upd.^2; dims=(nd2-1, nd2)) .+ ε)
    upd  = upd .* (vn ./ (vn2 .+ ε))
...
function setup_optimizer(model, args; lr_override=nothing)
    ...
    wd = Float32(args["wd"])
    β1 = Float32(args["muon-beta"])
    β2 = Float32(args["muon-beta2"])
    ns = Int(args["muon-ns-steps"])
    opt_rule = NorMuon(; lr=base_lr, beta1=β1, beta2=β2, ns_steps=ns, wd=wd)
    return Optimisers.setup(opt_rule, model)
end

# AdamW simplie for comparizion:
#function setup_optimizer(model, args; lr_override=nothing)
 #   base_lr = lr_override === nothing ? Float32(args["lr"]) : Float32(lr_override)
  #  wd = Float32(args["wd"])
...
function save_ckpt(path, model, opt, step::Int, best_loss::Float32, loader::ByteLoader, args; note="")
    mkpath(dirname(path))
    model_cpu = cpu(model)
    opt_cpu = Functors.fmap(opt) do v; HAS_CUDA && v isa CuArray ? Array(v) : v; end
    lbuf, lpos = loader_state_for_save(loader)
    JLD2.jldsave(path; model_state=Flux.state(model_cpu), opt_state=opt_cpu, step=step,
        best_loss=best_loss, loader_buf=Vector{Int32}(lbuf), loader_pos=Int(lpos),
        vocab=VOCAB, note=note, saved_at=string(now()))
    return path
end

save_latest!(dir, m, o, s, b, l, a; note="") = (p=ckpt_latest(dir); save_ckpt(p,m,o,s,b,l,a;note=note); cp(p,ckpt_step(dir,s);force=true); p)
save_good!(dir, m, o, s, b, l, a; note="")   = (p=ckpt_latest_good(dir); save_ckpt(p,m,o,s,b,l,a;note=note); cp(p,ckpt_step_good(dir,s);force=true); p)
save_best!(dir, m, o, s, b, l, a; note="")   = save_ckpt(ckpt_best(dir), m, o, s, b, l, a; note=note)
save_bad!(dir, m, o, s, b, l, a; note="")    = save_ckpt(ckpt_step_bad(dir, s), m, o, s, b, l, a; note=note)
...
function load_ckpt_state(path)
    ...
    ckvocab != 0 && ckvocab != VOCAB && @warn "Checkpoint vocab=$ckvocab differs from current VOCAB=$VOCAB"
    return st, opt_state, lbuf, lpos, step, best
end

function rollback_to_good!(dir, model_cpu, loader, args)
    path = ckpt_latest_good(dir)
    !isfile(path) && (@warn "No latest_good checkpoint for rollback: $path"; return nothing)
...
function generate_sample(model, prompt::String; ctx_len::Int, max_new::Int, greedy::Bool=false, topk::Int=40, topp::Float64=0.9, temp::Float64=0.8)
    ...
    for _ in 1:max_new
        ...
        any(!isfinite, lv) && (push!(toks, Int32(EOS_TOKEN)); break)
...
function suggest_loader_target_tokens(cpu_cache::CPUCacheInfo, args)
    l3 = cpu_cache.ok && cpu_cache.l3>0 ? cpu_cache.l3 : 8*1024^2
...
function microprobe_tokps(model_dev, seq::Int, batch::Int; iters::Int=3)
    ...
    l, grads = Zygote.withgradient(model_dev) do m
        logits = m(x)
        # stable_nll_loss_fp16 handles FP32 conversion internally
        stable_nll_loss_fp16(reshape(logits, VOCAB, :), reshape(y, :))
    end

    _ = grads[1]; HAS_CUDA && CUDA.synchronize(); t0 = time_ns()
    for _ in 1:iters
        l, grads = Zygote.withgradient(model_dev) do m; logits = m(x); stable_nll_loss_fp16(reshape(logits,VOCAB,:), reshape(y,:)); end
...
function autotune!(args, gpu::GPUInfo, cpu_cache::CPUCacheInfo)
    ...
    for (s,b,a) in pairs
        ...
        if args["fp16"] && HAS_CUDA; model_cpu = Functors.fmap(model_cpu) do v; v isa AbstractArray ? safe_fp16_convert(v) : v; end; end
        model = model_cpu |> DEV
...
function run_unit_tests(model, args)
    ...
    test_ok = try
        ...
        targets_t = DEV(ones(Int32, seq_t * batch_t)); loss_t = to_scalar(stable_nll_loss_fp16(zero_logits, targets_t))
    ...
    test2_ok = try
        ...
        logits_t = model(x_t); loss_t = to_scalar(stable_nll_loss_fp16(reshape(logits_t,VOCAB,:), reshape(y_t,:)))
        reasonable = isfinite(loss_t) && 3.0f0 < loss_t < 15.0f0
...
# ============================================================
# Исправленный smoke_test! съ обходомъ Zygote-бага
# ============================================================
function smoke_test!(model, args)
        ...
        try
            ...
            CUDA.@allowscalar begin
                l, grads = Zygote.withgradient(model) do m
                    ...
                    logits_f32 = Float32.(logits)
                    logits_clamped = clamp.(logits_f32, -100f0, 100f0)
                    stable_nll_loss_fp16(reshape(logits_clamped, VOCAB, :), reshape(y_t, :))
                end
            end
            
            # 🔑 Безопасное извлеченіе безъ Array(::Float32)
            lf = to_scalar(l)  # ✅ ИСПРАВЛЕНИЕ
    ...
    println("🧪 Starting final smoke test step...")
    seq_s = min(32, args["seq"]); batch_s = 1
    ok = try
        println("[DEBUG 1] Генерація входныхъ тензоровъ...")
        x_s = DEV(rand(Int32(1):Int32(VOCAB), seq_s, batch_s))
        y_s = DEV(rand(Int32(1):Int32(VOCAB), seq_s, batch_s))

        println("[DEBUG 2] Иніціализація оптимизатора...")
        opt_s = setup_optimizer(model, args)

        println("[DEBUG 3] Запускъ forward + backward (Zygote.withgradient)...")
        # 🔑 КРИТИЧЕСКІЙ ПАТЧЪ: обходъ Zygote scalar-indexing бага
        CUDA.allowscalar(true)
        l_s, grads = try
            Zygote.withgradient(model) do m
                logits = m(x_s)
                logits_f32 = Float32.(logits)
                logits_clamped = clamp.(logits_f32, -100f0, 100f0)
                stable_nll_loss_fp16(reshape(logits_clamped, VOCAB, :), reshape(y_s, :))
            end
        finally
            CUDA.allowscalar(false)
        end

        println("[DEBUG 4] Извлеченіе loss черезъ to_scalar...")
        loss_val = try
            to_scalar(l_s)  # ✅ ИСПРАВЛЕНИЕ
        catch e
            @warn "to_scalar failed: $e, пытаюсь прямое извлеченіе"
            if l_s isa Number
                Float32(l_s)
            elseif l_s isa AbstractArray
                Float32(Array(l_s)[1])
            else
                Float32(0)
            end
        end
        println("[DEBUG 4] ✅ Loss extracted: $loss_val")

        println("[DEBUG 5] Провѣрка градіентовъ на конечность...")
        gs_s = grads[1]
        grad_ok = tree_all_finite(gs_s)
        println("[DEBUG 5] ✅ Gradients finite: $grad_ok")

        println("[DEBUG 6] Шагъ оптимизатора (Optimisers.update)...")
        _, model = Optimisers.update(opt_s, model, gs_s)
        HAS_CUDA && CUDA.synchronize()
        println("[DEBUG 6] ✅ Optimizer step done")

        println("[DEBUG 7] Финальная провѣрка isfinite(loss_val)...")
        result = isfinite(loss_val)
        println("[DEBUG 7] ✅ Final check passed: $result")
        
        result
    catch e
        println("❌ Smoke test FAILED! Ошибка возникла послѣдней успешной метки выше.")
        println("📜 Stacktrace:")
        println(sprint(showerror, e))
        false
    end

    ok || error("Smoke test fail.")
...
function run_quick_check(model, args, loader)
    ...
    for step in 1:5
        ...
        l, grads = Zygote.withgradient(model) do m; logits = m(x_in); stable_nll_loss_fp16(reshape(logits,VOCAB,:), reshape(y_tgt,:)); end
        lf = to_scalar(l); push!(losses, lf); gs = grads[1]; gnorm = grad_l2norm(gs)
...
function train!(model_dev, model_cpu_ref, args, loader, start_step::Int, best::Float32; opt_loaded=nothing, model_master=nothing)
    ...
    # === Очистка VRAM передъ началомъ цикла ===
    GC.gc(false)
    HAS_CUDA && CUDA.reclaim()
    HAS_CUDA && CUDA.synchronize()
    @info "🧹 VRAM cleared before training loop. Free: $(round(CUDA.available_memory()/1e9, digits=2)) GB"

    # === Гарантія безопаснаго отката ===
    if !isfile(ckpt_latest_good(args["ckpt-dir"]))
        @info "💾 Saving initial latest_good checkpoint for safe rollback..."
        try
            save_good!(args["ckpt-dir"], model_dev, opt, start_step-1, best, loader, args; note="initial_safe")
        catch e
            @warn "Failed to save initial latest_good: $e"
        end
    end


    # ---- FP32 warmup initialisation ----
    warmup_steps_fp32 = args["warmup"]   # use full warmup period
    current_precision = :fp32
    if args["fp16"] && warmup_steps_fp32 > 0
        @info "Starting training in FP32 for first $warmup_steps_fp32 steps (warmup)"
        # Convert model to FP32
        to_fp32(x) = x isa AbstractArray ? Float32.(x) : x
        model_dev = Functors.fmap(to_fp32, model_dev)
        # Recreate optimizer for FP32 parameters using base LR (adjust! will update it later)
        opt = setup_optimizer(model_dev, args; lr_override=Float32(args["lr"]))
        model_ema = deepcopy(model_dev)
        # Also convert CPU reference model
        model_cpu_ref = Functors.fmap(to_fp32, model_cpu_ref)
    else
    ...
    # ---- FP32 → FP16 switch after warmup ----
#if args["fp16"] && current_precision == :fp32 && step >= warmup_steps_fp32
local_step = step - start_step + 1

if args["fp16"] && current_precision == :fp32 && local_step > warmup_steps_fp32

    @info "Switching model from FP32 to FP16 at step $step"
    
    # Safe conversion: clamp to [-5e4, 5e4] to avoid overflow in Float16
    function safe_fp16(x)
        if x isa AbstractArray
            # Clamp to ±50000 (safe margin below 65504)
            clamped = clamp.(x, -50000f0, 50000f0)
            return Float16.(clamped)
        else
            return x
        end
    ...
    model_dev = Functors.fmap(safe_fp16, model_dev)
    
    # Optional: verify no Inf/NaN after conversion
    if !tree_all_finite(model_dev)
        @error "Model contains Inf/NaN after FP16 conversion – aborting switch"
        return model_dev, model_ema, best
    end
    
    opt = setup_optimizer(model_dev, args; lr_override=effective_lr)
    model_ema = deepcopy(model_dev)
    model_cpu_ref = Functors.fmap(safe_fp16, model_cpu_ref)
    current_precision = :fp16
    @info "Successfully switched to FP16, all parameters finite"
end    
    # -----------------------------------------
...
for _ in 1:args["accum"]
    ...
    # 🛡️ Check inputs for NaNs/Infs
    if any(!isfinite, x_in) || any(!isfinite, y_tgt)
        @warn "NaN/Inf in input batch; skipping micro-step"
        local_bad = true
        local_reason = "input batch contains NaN/Inf"
        break
    end

    # Forward + backward (stable FP16 loss)
    l, grads = Zygote.withgradient(model_dev) do m
        logits = m(x_in)
        # stable_nll_loss_fp16 handles FP32 conversion + clamping internally
        stable_nll_loss_fp16(reshape(logits, VOCAB, :), reshape(y_tgt, :))
    end

    lf = to_scalar(l)  # ✅ ИСПРАВЛЕНИЕ
    if !isfinite(lf)
        local_bad = true
        local_reason = "micro loss NaN/Inf"
        break
    end

    g = grads[1]
    g_fp32 = Functors.fmap(g) do v
        v isa AbstractArray{Float16} ? Float32.(v) : v
    end

    if !tree_all_finite(g_fp32)
        local_bad = true
        local_reason = "gradient NaN/Inf"
        break
    end

            step_loss += lf
            gs_sum = gs_sum === nothing ? g_fp32 : add_grads(gs_sum, g_fp32)
        end        

        # === FP16 Stability Fix: Приведеніе градіентовъ къ FP32 передъ клиппингомъ ===
        gs = div_grads(gs_sum, args["accum"])
        gs_fp32 = Functors.fmap(gs) do g
            g isa AbstractArray{Float16} ? Float32.(g) : g
        end
        ...
        # Клиппингъ и обновленіе работаютъ съ FP32 градіентами, избѣгая переполненія
        gs, gnorm_post, gnorm_pre = adaptive_clip_grads(gs_fp32, args)
...
function internal_preflight!()
    required = [:nll_loss, :stable_nll_loss_fp16, :grad_l2norm, :scale_grads, :add_grads, :div_grads, :adaptive_clip_grads, :update_qat_control!, :smoke_test!, :train!,
        ...
        :token_to_cyr2, :probe_tokenizer, :sample_topk_topp, :generate_sample, :to_scalar, :safe_fp16_convert]
    missing = Symbol[]
...
function main()
    ...
    @info "🔧 Detected Pascal (SM 6.1). Forcing FP32 (flash attention preserved)."
    args["fp16"] = false; args["no-fp16"] = true
    # attn НЕ трогаем — NNkernels.flash_attention работает в FP32 на SM 6.x

        elseif cc.major == 7 && cc.minor == 5
            @info "🔧 Detected Turing (SM 7.5). Forcing FP16 and FlashAttention."
            ENV["CUDA_DISABLE_TENSOR_CORES"] = "1"  # ← Измѣнено
        end
    end

    hd = div(args["dim"], args["heads"])
    
# claude-sonnet-4-6-search: Убрать дублирующую проверку из main() — теперь она в validate_model_config!
# if args["attn"] == "flash"; ispow2(hd) || error("flash requires head_dim=dim/heads to be power-of-two. head_dim=$hd"); end
    ...
    if !isempty(args["resume"])
        ...
        if opt_loaded !== nothing
    @info "⚠️ Optimizer state discarded: switching to NorMuon hybrid (Muon for 2D+, AdamW-style for 1D)" #        @info "⚠️ Optimizer state discarded: switching to hybrid rule tree (NorMuon + AdamW)"
            opt_loaded = nothing
        end
    end

function validate_model_config!(args)
    dim   = args["dim"]
    heads = args["heads"]
    kv    = args["kv-heads"]
    attn  = args["attn"]

    # 1. dim делится на heads?
    if dim % heads != 0
        valid = filter(h -> dim % h == 0, 1:dim)
        error("""
        ❌ dim=$dim не делится на heads=$heads (head_dim=$(dim/heads) не целое).
        Допустимые значения heads для dim=$dim:
          $(join(valid, ", "))
        """)
    end
    ...
    # 2. heads делится на kv-heads?
    if heads % kv != 0
        valid_kv = filter(k -> heads % k == 0, 1:heads)
        error("""
        ❌ heads=$heads не делится на kv-heads=$kv.
        Допустимые kv-heads для heads=$heads:
          $(join(valid_kv, ", "))
        """)
    end

    # 3. Flash требует head_dim = 2^n
    if attn == "flash" && !ispow2(hd)
        valid_flash = filter(h -> dim % h == 0 && ispow2(div(dim, h)), 1:dim)
        error("""
        ❌ flash attention требует head_dim = степень двойки, но head_dim=$hd.
        Допустимые heads для flash+dim=$dim:
          $(join(valid_flash, ", "))
          (head_dim: $(join(map(h -> div(dim,h), valid_flash), ", ")))
        """)
    end

    @info "✅ Config OK: dim=$dim heads=$heads kv=$kv head_dim=$hd attn=$attn"
    return nothing
end

   # ← ДОБАВИТЬ ЗДЕСЬ, после detect_gpu() и до build_model_cpu()
    ...
    if args["fp16"] && HAS_CUDA
        model_cpu = Functors.fmap(model_cpu) do v
            v isa AbstractArray ? safe_fp16_convert(v) : v  # ← Измѣнено
        end
    end
    model = model_cpu |> DEV
    ...
    if get(args,"autotune-applied",false) && args["seq"] != old_seq
        ...
        if args["fp16"] && HAS_CUDA; new_model_cpu = Functors.fmap(new_model_cpu) do v; v isa AbstractArray ? safe_fp16_convert(v) : v; end; end
    ...
    if args["skip-smoke-test"]
        @info "⏭️ Smoke test skipped (--skip-smoke-test)"
    else
