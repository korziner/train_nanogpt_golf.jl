# NanoGPT-Golf v5.5 — Hybrid tokenizer + Z-Loss + 8-bit NorMuon + Hybrid Checkpoint

using Flux, NNlib, Optimisers, Zygote, Functors
using Flux: params
using ArgParse, JLD2, JSON3
using LinearAlgebra, Statistics, Random, Printf, Dates
using Base.Sort: partialsort      # for top‑k sampling
using CUDA
...
# ============================================================
# 8-bit Optimizer Quantization
# ============================================================
function quantize_to_int8(x::AbstractArray; per_row::Bool=true)
    if per_row && ndims(x) >= 2
        mx = maximum(abs, Float32.(x); dims=Tuple(2:ndims(x)))
        scale = vec(mx) ./ 127.0f0
        scale = max.(scale, 1f-8)
        scale_shape = [size(x, 1), ones(Int, ndims(x)-1)...]
        x_scaled = Float32.(x) ./ reshape(scale, scale_shape...)
        x_int8 = Int8.(clamp.(round.(x_scaled), -127.0f0, 127.0f0))
        return x_int8, Float16.(scale)
    else
        scale = Float32(maximum(abs, x)) / 127.0f0
        scale = max(scale, 1f-8)
        x_scaled = Float32.(x) / scale
        x_int8 = Int8.(clamp.(round.(x_scaled), -127.0f0, 127.0f0))
        return x_int8, Float16(scale)
    end
end

function dequantize_from_int8(x_int8::AbstractArray{Int8}, scale; per_row::Bool=true)
    if per_row && ndims(x_int8) >= 2
        scale_shape = [size(x_int8, 1), ones(Int, ndims(x_int8)-1)...]
        return Float16.(Float32.(x_int8) .* reshape(Float32.(scale), scale_shape...))
    else
        return Float16.(Float32.(x_int8) .* Float32(scale))
    end
end
...
# ============================================================
# Optimizers (NorMuon & NorMuon8bit)
# ============================================================
...
struct NorMuon8bit <: Optimisers.AbstractRule
    eta::Float32; beta1::Float32; beta2::Float32; ns_steps::Int; wd::Float32
    scale_update_every::Int; per_row::Bool
end
NorMuon8bit(; lr=6f-4, beta1=0.95f0, beta2=0.95f0, ns_steps=5, wd=0f0, scale_update_every=16, per_row=true) =
    NorMuon8bit(Float32(lr), Float32(beta1), Float32(beta2), Int(ns_steps), Float32(wd), Int(scale_update_every), Bool(per_row))

struct NM8State{M8,MS,V8,VS,S}
    m_int8::M8; m_scale::MS; v_int8::V8; v_scale::VS; step::S
end

function Optimisers.init(o::NorMuon8bit, x::AbstractArray)
    nd = ndims(x)
    m_int8 = zeros(Int8, size(x))
    v_int8 = zeros(Int8, nd >= 2 ? ntuple(i -> i==nd ? 1 : size(x,i), nd) : size(x))
    if o.per_row && nd >= 2
        m_scale = ones(Float16, size(x, 1))
        v_scale = ones(Float16, size(v_int8, 1))
    else
        m_scale = Float16(1.0); v_scale = Float16(1.0)
    end
    HAS_CUDA && x isa CuArray && begin
        m_int8 = cu(m_int8); v_int8 = cu(v_int8)
        m_scale = cu(m_scale); v_scale = cu(v_scale)
    end
    return NM8State(m_int8, m_scale, v_int8, v_scale, 0)
end

function Optimisers.apply!(o::NorMuon8bit, st::NM8State, x::AbstractArray, dx)
    dx === nothing && return st, zero(x)
    T = eltype(x); dx = T.(dx); nd = ndims(x); η = T(o.eta); step = st.step + 1
    m = T.(dequantize_from_int8(st.m_int8, st.m_scale; per_row=o.per_row))
    v = T.(dequantize_from_int8(st.v_int8, st.v_scale; per_row=o.per_row))
    
    if nd < 2
        m_new = T(o.beta1) .* m .+ T(1 - o.beta1) .* dx
        upd = m_new
        o.wd > 0f0 && (upd = upd .+ T(o.wd) .* x)
        m_int8_new, m_scale_new = quantize_to_int8(Float16.(m_new); per_row=o.per_row)
        return NM8State(m_int8_new, m_scale_new, st.v_int8, st.v_scale, step), η .* upd
    end
    
    β1, β2, ε = T(o.beta1), T(o.beta2), T(1e-4)
    m_new = β1 .* m .+ (T(1) - β1) .* dx
    upd   = β1 .* dx  .+ (T(1) - β1) .* m_new
    osz = nd == 4 ? size(upd) : nothing
    osz !== nothing && (upd = reshape(upd, size(upd, 1), :))
    upd = T.(zeropower_ns5(upd; steps=o.ns_steps))
    ...
    m_int8_new, m_scale_new = quantize_to_int8(Float16.(m_new); per_row=o.per_row)
    v_int8_new, v_scale_new = quantize_to_int8(Float16.(v_new); per_row=o.per_row)
    return NM8State(m_int8_new, m_scale_new, v_int8_new, v_scale_new, step), η .* upd
end

function convert_optimizer_state_to_8bit(opt_state)
    return Functors.fmap(opt_state) do st
        if st isa NMState
            m_int8, m_scale = quantize_to_int8(Float16.(st.m); per_row=true)
            v_int8, v_scale = quantize_to_int8(Float16.(st.v); per_row=true)
            return NM8State(m_int8, m_scale, v_int8, v_scale, 0)
        else
            return st
        end
...
function convert_optimizer_state_from_8bit(opt_state)
    return Functors.fmap(opt_state) do st
        if st isa NM8State
            m = dequantize_from_int8(st.m_int8, st.m_scale; per_row=true)
            v = dequantize_from_int8(st.v_int8, st.v_scale; per_row=true)
            return NMState(m, v)
        else
            return st
        end
...
function setup_optimizer(model, args; lr_override=nothing)
    ...
    if get(args, "optimizer-8bit", false)
        @info "🔧 Using NorMuon8bit optimizer"
        opt_rule = NorMuon8bit(lr=base_lr, beta1=Float32(args["muon-beta"]), beta2=Float32(args["muon-beta2"]),
                               ns_steps=args["muon-ns-steps"], wd=Float32(args["wd"]),
                               scale_update_every=get(args, "optimizer-8bit-scale-update-every", 16), per_row=true)
    else
...
function save_ckpt(path, model, opt, step::Int, best_loss::Float32, loader::ByteLoader, args; note="", model_master=nothing)
    mkpath(dirname(path))
    model_cpu = cpu(model)
    master_cpu = model_master !== nothing ? cpu(model_master) : nothing
    ...
    opt_type = get(args, "optimizer-8bit", false) ? "NorMuon8bit" : "NorMuon"
    arch_cfg = Dict{String,Any}("layers"=>args["layers"], "dim"=>args["dim"], "heads"=>args["heads"],
                                "kv-heads"=>args["kv-heads"], "ff-mult"=>args["ff-mult"], "vocab"=>VOCAB, "attn_mode"=>args["attn"])
    
    JLD2.jldsave(path; 
        model_state    = Flux.state(model_cpu),
        master_state   = master_cpu !== nothing ? Flux.state(master_cpu) : nothing,
        opt_state      = opt_cpu,
        step           = step,
        best_loss      = best_loss,
...
function load_ckpt_state(path; current_args=nothing)
    ck   = JLD2.load(path)
    st   = ck["model_state"]
    ...
    if opt_state !== nothing && current_args !== nothing && haskey(ck, "optimizer_type")
        ck_opt_type = ck["optimizer_type"]
        current_uses_8bit = get(current_args, "optimizer-8bit", false)
        if ck_opt_type == "NorMuon" && current_uses_8bit
            @info "🔄 Converting optimizer state: FP16/FP32 → 8-bit"
            opt_state = convert_optimizer_state_to_8bit(opt_state)
        elseif ck_opt_type == "NorMuon8bit" && !current_uses_8bit
            @info "🔄 Converting optimizer state: 8-bit → FP16/FP32"
            opt_state = convert_optimizer_state_from_8bit(opt_state)
        end
    end
...
function rollback_to_good!(dir, model_cpu, loader, args)
    ...
    st, _, lbuf, lpos, step, best = load_ckpt_state(path; current_args=args)
...
function analyze_tokens_health(ts::Vector{Int32}, text::String, args)
    ...
    trigram_div  = ngram_diversity(text, 3)  # fixed: was 2, should be 3 for trigram
    reasons = String[]
...
function smoke_test!(model, args)
    ...
    if HAS_CUDA && HYBRID_CKPT_ENABLED[] && !args["skip-hybrid-checkpoint-test"]
        ...
        if args["skip-hybrid-checkpoint-test"]
            println("  ⏭️ Skipping hybrid_checkpoint test (--skip-hybrid-checkpoint-test)")
        else
            println("  ⚠️ hybrid_checkpoint not available (CPU only or disabled)")
        end
    end
    # ... далее без изменений ...
...
function train!(model_dev, model_cpu_ref, args, loader, start_step::Int, best::Float32; opt_loaded=nothing, model_master=nothing)
    master_for_opt = model_master !== nothing ? model_master : model_dev
    if opt_loaded !== nothing
        opt = opt_loaded
        @info "✅ Loaded optimizer state from checkpoint"
    else
        opt = setup_optimizer(master_for_opt, args)
        @info "🔧 Optimizer initialized on $(eltype(first(params(master_for_opt)))) master weights"
    end
...
# В начале цикла for step in start_step:args["iters"] добавьте:
if step > start_step && step % 10 == 0
    if any(!isfinite, params(model_dev))
        @error "Model parameters contain NaN/Inf at step $step. Aborting."
...
if model_master !== nothing
    # 1. Градиенты на CPU → Float32
    gs_cpu = Functors.fmap(gs) do g
        g isa CuArray ? Array(g) : g
    end
    gs_fp32 = Functors.fmap(gs_cpu) do g
        g isa AbstractArray ? Float32.(g) : g
    end
    
    # 2. Обновление master (FP32)
    opt, model_master = Optimisers.update(opt, model_master, gs_fp32)
    
    # 3. Безопасное копирование master -> device (FP32 → FP16) с защитой от overflow
    function safe_copy_to_fp16!(dst_fp16, src_fp32)
        # Ограничиваем значения диапазоном Float16, чтобы избежать Inf
        src_clamped = clamp.(src_fp32, -65504f0, 65504f0)
        copyto!(dst_fp16, src_clamped)
    end
    
    master_arrays = Functors.fmap(x -> x isa AbstractArray ? x : nothing, model_master) |> Functors.flatten
    dev_arrays    = Functors.fmap(x -> x isa AbstractArray ? x : nothing, model_dev)    |> Functors.flatten
    
    for (m_arr, d_arr) in zip(master_arrays, dev_arrays)
        safe_copy_to_fp16!(d_arr, m_arr)
    end
    
    # Проверка: если в master появились Inf/NaN, сбросить состояние
    if any(!isfinite, m_arr for m_arr in master_arrays)
        @error "Master weights contain NaN/Inf after update! Rolling back to previous good checkpoint."
        # здесь можно вызвать rollback (уже есть в коде)
        # но для простоты – перезапустите обучение с --no-fp16
    end
end

        ema_beta < 1f0 && ema_update!(model_ema, model_dev, ema_beta)

     maybe_adjust_hybrid_checkpoint!(step, args["log-every"])
...
function main()
    ...
    if !isempty(args["resume"])
        ...
        st, opt_loaded, lbuf, lpos, resume_step, best = load_ckpt_state(args["resume"]; current_args=args)
    ...
    # Master weights in FP32 (CPU)
    model_master = deepcopy(model_cpu)
    ...
    if args["optimizer-8bit"]; println("Optimizer: NorMuon8bit (8-bit moments)"); else; println("Optimizer: NorMuon (FP16/FP32)"); end

    args["dry-run"] && (println("Dry-run done."); return)
    old_seq = args["seq"]; old_batch = args["batch"]
    autotune!(args, gpu, cpu_cache); println("Autotune: applied=", get(args,"autotune-applied",false))
    if get(args,"autotune-applied",false) && (args["seq"] != old_seq || args["batch"] != old_batch)
        println("Seq/batch changed by autotune, rebuilding model and loader...")
        new_model_cpu = build_model_cpu(args)
        # Recreate master from the new FP32 model
        model_master = deepcopy(new_model_cpu)
        ...
        GC.gc(false); HAS_CUDA && CUDA.reclaim()
        # Recreate loader with new seq/batch
        target_tokens = haskey(args,"loader-target-tokens") ? args["loader-target-tokens"] : 1_000_000
        loader = ByteLoader(args["data"], args["seq"], args["batch"]; target_tokens=target_tokens)
        if !isempty(lbuf); loader_restore!(loader, lbuf, lpos); end
    else
        target_tokens = haskey(args,"loader-target-tokens") ? args["loader-target-tokens"] : 1_000_000
        loader = ByteLoader(args["data"], args["seq"], args["batch"]; target_tokens=target_tokens)
