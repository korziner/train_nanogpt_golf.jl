...
to_scalar(x) = x isa Number ? Float32(x) : Float32(Array(x)[1])
...
function nll_loss(logits::AbstractMatrix, targets::AbstractVecOrMat{<:Integer})
    ...
    idx = flat_t .+ (0:N-1) .* V
    # Явное sum/length избѣгаетъ проблемъ диспетчеризаціи Statistics.mean на GPU
...
function tree_max_abs(x)::Float32
    ...
    Functors.fmap(x) do v
        if v isa AbstractArray && length(v) > 0
            try
                m = to_scalar(CUDA.maximum(abs, v))
                mx[] = max(mx[], m)
            catch _
                mx[] = Inf32
            end
...
@adjoint function hybrid_checkpoint(f, x)
    ...
    dt = (time_ns() - t0) / 1e9
    nbytes = compressed_bytes(method, payload)
    OFFLOAD_BYTES_TO_CPU[] += nbytes; OFFLOAD_TIME_CPU_S[] += dt; OFFLOAD_COUNT[] += 1
    ...
    function back(Δ)
        ...
        HAS_CUDA && CUDA.synchronize()
        dt_r = (time_ns() - t1) / 1e9
        OFFLOAD_BYTES_TO_GPU[] += nbytes; OFFLOAD_TIME_GPU_S[] += dt_r
        if HAS_CUDA
            try
                if any(!isfinite, Array(x_reloaded))
                    @warn "hybrid_checkpoint: $method — NaN/Inf въ возстановленныхъ данныхъ"
                    return (nothing, zero(x))
                end
            catch e
                @warn "hybrid_checkpoint: провѣрка не удалась: $e"
            end
        end
        ...
        if grads[1] !== nothing
            try
                if any(!isfinite, Array(grads[1]))
                    @warn "hybrid_checkpoint: NaN/Inf въ градіентахъ"
                    return (nothing, zero(x))
                end
            catch e
                @warn "hybrid_checkpoint: провѣрка градіентовъ не удалась: $e"
            end
        end
...
l, grads = Zygote.withgradient(model_dev) do m
    logits = m(x_in)
    logits_f32 = Float32.(logits)   # stable FP32 computation
    nll_loss(reshape(logits_f32, VOCAB, :), reshape(y_tgt, :))
end
...
function run_unit_tests(model, args)
    ...
    test_ok = try
        ...
        targets_t = DEV(ones(Int32, seq_t * batch_t)); loss_t = to_scalar(nll_loss(zero_logits, targets_t))
    ...
    test2_ok = try
        ...
        logits_t = model(x_t); loss_t = to_scalar(nll_loss(reshape(logits_t,VOCAB,:), reshape(y_t,:)))
        reasonable = isfinite(loss_t) && 3.0f0 < loss_t < 15.0f0  # Ожидаемъ ~5.95 ± допускъ # reasonable = isfinite(loss_t) && loss_t < 100.0f0
...
# ============================================================
# Вспомогательная функція (если ещё не объявлена)
# ============================================================
to_scalar(x) = x isa Number ? Float32(x) : Float32(Array(x)[1])
...
function smoke_test!(model, args)
    ...
    if HAS_CUDA && HYBRID_CKPT_ENABLED[] && !get(args, "skip-hybrid-checkpoint-test", false)
        ...
        try
            ...
            lf = l isa Number ? Float32(l) : Float32(Array(l)[1])
...
function train!(model_dev, model_cpu_ref, args, loader, start_step::Int, best::Float32; opt_loaded=nothing, model_master=nothing)
    ...
    if args["fp16"] && current_precision == :fp32 && step >= warmup_steps_fp32
        ...
        function _to_fp16(x)
            x isa AbstractArray ? Float16.(x) : x
        end
        model_dev = Functors.fmap(_to_fp16, model_dev)
        # Recreate optimizer for FP16 parameters (reset states)
        opt = setup_optimizer(model_dev, args; lr_override=effective_lr)
        model_ema = deepcopy(model_dev)
        model_cpu_ref = Functors.fmap(_to_fp16, model_cpu_ref)
        current_precision = :fp16
    end
    # -----------------------------------------

        gs_sum = nothing; step_loss = 0f0; local_bad = false; local_reason = ""
# Load one full accumulated batch (args["accum"] * batch_size) – you already do that via loader
# For each micro-step, you already call next_batch! once.
# So simply add the check right after loading each micro-batch:
...
for _ in 1:args["accum"]
    ...
    # ... rest of the micro-step (loss, gradients, etc.)

    # Forward + backward
    l, grads = Zygote.withgradient(model_dev) do m
        logits = m(x_in)
        # Convert to FP32 and clamp to avoid overflow in softmax
        logits_f32 = Float32.(logits)
        logits_clamped = clamp.(logits_f32, -100f0, 100f0)   # safe range
        nll_loss(reshape(logits_clamped, VOCAB, :), reshape(y_tgt, :))
    end
